...
def add_word_in_user_dict(request):
    user_word = NewUserWord(user=request.user, user_word=request.POST["user_word"])
...
class RegisterUser(MixinDataParams, CreateView):
    ...
    def form_valid(self, form):
        ...
        DatesLastAddedWordInUserDict.objects.create(
...
class Training(MixinDataParams, TemplateView):
    ...
    def get_context_data(self, **kwargs: Any) -> Dict[str, Any]:
        try:
            user_dict = list(
                UserDictionaries.objects.filter(user=self.request.user)[:5]
            )
...
def result_training(request):
    ...
    def update_user_dict(answer_list, is_true_answer):
        for i in answer_list:
            MasterDictionaries.objects.get(word=i).userdictionaries_set.filter(
                user=request.user
            ).update(
